...
int main(int argc, char** argv){
	...
	char prefix[20];
	strcpy(prefix,"test_from_FILE_2");
    ...
    while (!file.eof()) {
        ...
        {	
			...
			fprintf(fp,"%lu %lu\n",par->exha_solution.size(),par->vidca_solution.size());
        ...
        cout << "Time taken by "<<REP<<" reps.: "
            << durTotalExhaustive+durTotalVidca<< " microseconds" << endl; 
		...
		cout<<"-----------------------------------------------------------------------------------------------"<<endl;
        fprintf(fp,"%f %f\n",cardinalidadExhaustive/REP,cardinalidadVidca/REP);
    }
