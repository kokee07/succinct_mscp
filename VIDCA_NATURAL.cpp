...
void analyzeF(ParProg *par)
{
	...
	int pos = 0;
	for (pair<int, int> values : valor_cant)
	{ // iter through pair elements
	...
	auto stop = high_resolution_clock::now();
	if (PRINT)
		cout << "Subsets analyzed in " << (duration_cast<microseconds>(stop - start)).count() << " microseconds." << endl;
}
...
float estimatePC(ParProg *par)
{
	int famCardinality = par->F_sets.size();
	float partialSum = 0;
	for (int cont = 0; cont < par->chiSize; cont++)
	{
		item ind = par->P[cont];
		partialSum += ind.repetitions / float(famCardinality);
	}
...
void preSetCover(ParProg *par)
{
	...
	while (par->P[pos].repetitions == 1)
	{
		...
		if (!checkBit(par->I.mem, par->chi_map[par->P[pos].value]))
		{
			...
			if (CHECK)
				cout << "adding set: " << actual_set << endl;
			for (int otherVal : actual)
			{
				if (!checkBit(par->I.mem, par->chi_map[otherVal]))
				{ // If the element isn't already included, include it
					if (CHECK)
						cout << "    value: " << otherVal << endl;
	...
	if (!pos)
		cout << "Ningun elemento agregado en H1" << endl;
	if (PRINT)
	{
		cout << "---------------------------------------------------------" << endl;
	}
	...
	while (indice < par->chiSize && (int(par->P[indice].inSet.size()) == numSets))
	{
		...
		if (!checkBit(par->I.mem, par->chi_map[value]))
		{
			for (int posibleSet : par->P[indice].inSet)
			{
				...
				if (CHECK)
					cout << "> " << posibleSet << ", suma parcial: " << sumAct << endl;
				if (sumAct > bestSuma)
				{
					bestSuma = sumAct;
