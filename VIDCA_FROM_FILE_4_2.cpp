...
pair<int,int> findCandidate(ParProg* par, int index){
	...
	//<Creating binary mask of minimal elements.
	int sizeWord=par->I.nW;
	ulong* minMask =new ulong[sizeWord];
	for (int j = 0; j < sizeWord; j++)
	{
		minMask[j]=0;
	}
	while(indice2<par->chiSize && (par->P[indice2].inSet).size()==numSets){
		value=par->chi_map[par->P[indice2].value];
		if(!checkBit(par->I.mem,value)) setBit64(minMask,value);
		indice2++;	
	}
		...
		if(!checkBit(par->I.mem,par->chi_map[value])){
			for(int posibleSet:par->P[indice].inSet){
				...
				for (int pos = 0; pos < par->I.nW; pos++)
				{
					...
					ulong e = toCheck[pos]&(minMask[pos]);//&(~(par->I.mem[pos]));
