...
int main(int argc, char** argv){
	if(argc != 3){
		cout << "Execution Error! call: ./vidca <ns> <MAX_val>" << endl;
		exit(EXIT_FAILURE);
	}
	srand (time(NULL));
	int n = atoi(argv[1]);
...
void greedyAlg(ParProg *par){
    ...
    while(chi.size()>0){
		if(CHECK) cout<<"Lengt: "<<candidates.size()<<endl;
...
void preSetCover(ParProg *par){
	...
	while(par->P[pos].repetitions==1){
		...
		if(!getNum64(par->M.mem,actual_set,1)){ // If the set isn't already included, include it.
	...
	if(TEST){
		if(par->last_visited){
			cout<< "(H1) Elements of Chi already covered: "<<par->last_visited<<endl;
